...
function doIt(){
    var udata=null;
    database.ref("/"+getCookie("username")+"/Tasks").on("value",function(data){
      udata=data.val();
      if(udata===null||udata===undefined){
        database.ref("/"+getCookie("username")+"/").set({
        Tasks:0
        })
    }
    })  
...
setInterval(function(){
    ...
    database.ref("/"+getCookie("username")+"/Tasks").on("value",function(data){
        ...
        for(var i=num;i>0;i--){
            database.ref("/"+getCookie("username")+"?Task/Task"+i+"/Task").on("value",function(data){
                myTask=data.val();
            })
            database.ref("/"+getCookie("username")+"?Task/Task"+i+"/date").on("value",function(data){
                myDate=data.val();
            })
            if(myTask!==null){
...
function rem(numb){
database.ref(getCookie("username")+"?Task/Task"+numb).remove();
}